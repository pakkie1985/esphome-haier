...
private:
    ...
    byte power_command[17]     = {0xFF,0xFF,0x0C,0x40,0x00,0x00,0x00,0x0,0x0,0x01,0x5D,0x01,0x00,0x01,0xAC,0xBD,0xFB};
...
public:
    ...
    void setup() override {
        ...
        delay(1000);
		uart_->write_array(initialization_1, sizeof(initialization_1));
        auto raw = getHex(initialization_1, sizeof(initialization_1));
        ESP_LOGD("Haier", "initialization_1: %s ", raw.c_str());
		delay(1000);
		uart_->write_array(initialization_2, sizeof(initialization_2));
    ...
    void loop() override  {
        ...
        if (uart_->available() > 0) {
			if (uart_->read_byte(&data[0]) && data[0] != 0xFF) return;
			if (uart_->read_byte(&data[1]) && data[1] != 0xFF) return;
			
			data[0] = 0xFF;
			data[1] = 0xFF;

            uart_->read_array(data+2, sizeof(data)-2);
    ...
    void update() override {
        ...
        uart_->write_array(poll, sizeof(poll));
...
protected:
    esphome::climate::ClimateTraits traits() override {
		...
		traits.set_supported_modes({esphome::climate::CLIMATE_MODE_HEAT_COOL, esphome::climate::CLIMATE_MODE_HEAT, esphome::climate::CLIMATE_MODE_COOL, esphome::climate::CLIMATE_MODE_DRY, esphome::climate::CLIMATE_MODE_FAN_ONLY, esphome::climate::CLIMATE_MODE_OFF});

		traits.set_supported_fan_modes({esphome::climate::CLIMATE_FAN_ON, esphome::climate::CLIMATE_FAN_OFF, esphome::climate::CLIMATE_FAN_AUTO, esphome::climate::CLIMATE_FAN_LOW, esphome::climate::CLIMATE_FAN_MEDIUM, esphome::climate::CLIMATE_FAN_MIDDLE, esphome::climate::CLIMATE_FAN_HIGH});
		...
		traits.set_supported_swing_modes({esphome::climate::CLIMATE_SWING_OFF, esphome::climate::CLIMATE_SWING_BOTH, esphome::climate::CLIMATE_SWING_VERTICAL, esphome::climate::CLIMATE_SWING_HORIZONTAL});
        return traits;
    }
...
public:
    ...
    void parse_status() {
        ...
        if (GetPowerStatus() == false) {
            mode = esphome::climate::CLIMATE_MODE_OFF;
		} else {
			// Check current hvac mode
            switch (GetHvacModeStatus()) {
                case MODE_COOL:
                    mode = esphome::climate::CLIMATE_MODE_COOL;
                    break;
                case MODE_HEAT:
                    mode = esphome::climate::CLIMATE_MODE_HEAT;
                    break;
                case MODE_DRY:
				    mode = esphome::climate::CLIMATE_MODE_DRY;
					break;
				case MODE_FAN:
                    mode = esphome::climate::CLIMATE_MODE_FAN_ONLY;
                    break;
                case MODE_AUTO:
                default:
			...
			if ( GetQuietModeStatus() == true) {
                fan_mode = esphome::climate::CLIMATE_FAN_LOW;
            }
			// If we detect that fast mode is on the we read it as "fan high"
			else if( GetFastModeStatus() == true) {
				fan_mode = esphome::climate::CLIMATE_FAN_HIGH;
			}			
			else {				
				// No quiet or fast so we read the actual fan speed.
                switch (GetFanSpeedStatus()) {
                    case FAN_AUTO:
                        fan_mode = esphome::climate::CLIMATE_FAN_AUTO;
                        break;
                    case FAN_MID:
                        fan_mode = esphome::climate::CLIMATE_FAN_MEDIUM;
                        break;
                    //case FAN_MIDDLE:
                    //    fan_mode = esphome::climate::CLIMATE_FAN_MIDDLE;
                    //    break;
					case FAN_LOW:
						fan_mode = esphome::climate::CLIMATE_FAN_LOW;
                        break;
			...
			if( (GetHorizontalSwingStatus() == HORIZONTAL_SWING_AUTO) && (GetVerticalSwingStatus() == VERTICAL_SWING_AUTO) ){
				swing_mode = esphome::climate::CLIMATE_SWING_BOTH;				
			}
			else if(GetHorizontalSwingStatus() == HORIZONTAL_SWING_AUTO){
				swing_mode = esphome::climate::CLIMATE_SWING_HORIZONTAL;
			}
			else if(GetVerticalSwingStatus() == VERTICAL_SWING_AUTO){
				swing_mode = esphome::climate::CLIMATE_SWING_VERTICAL;
			}
			else{
				swing_mode = esphome::climate::CLIMATE_SWING_OFF;
			}
		}
    ...
    void control(const esphome::climate::ClimateCall &call) override {
        ...
        if (call.get_mode().has_value()) {
			...
			//if((new_mode != esphome::climate::CLIMATE_MODE_OFF) && (GetPowerStatus() == false)){
		...
		if (call.get_target_temperature().has_value()) {
			...
			control_command[SET_POINT_OFFSET] = (uint8_t) temp - 16;
			sendData(control_command, sizeof(control_command));			
    ...
    void sendData(byte * message, byte size) {
        ...
        uart_->write_array(message, size);
    ...
    std::string getHex(byte * message, byte size) {
        ...
        // raw.toUpperCase(); Not needed in C++
    ...
    uint16_t crc16(uint16_t crc, unsigned char *buf, size_t len)
    {
        while (len--) {
            crc ^= *buf++;
